...
app = FastAPI(title="A股量化工具", version="13.0.23", docs_url="/docs", redoc_url="/redoc")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def startup() -> None:
    ...
    try:
        from .services import stock_ai as stock_ai_svc
        stock_ai_svc.ensure_tables()
    except Exception as exc:  # noqa: BLE001
        log.warning("个股AI诊断表初始化失败: %s", exc)
    # 2) 默认自选股
