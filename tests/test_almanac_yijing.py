"""13.0.28：易经/大六壬/六爻卜卦匹配/奇门旺相选股。不编造个股，不写入智能选股名单。"""
...
class AlmanacYijingTests(unittest.TestCase):
    ...
    def tearDown(self):
        for code in (self.CODE_DIV, self.CODE_OK, self.CODE_DEAD, self.CODE_LOW):
    ...
    def test_cast_six_and_match_only_local(self):
        ...
        out = yijing.result_from_yaos(yaos, date(2026, 8, 16), 12)
        ...
        self.assertIn(self.CODE_DIV, codes)
        self.assertNotIn("sz999999", codes)
    ...
    def test_divination_invalid_date(self):
        ...
        rng = random.Random(1)
        ok = yijing.divination("2026-08-16", 12, rng)
